...
class MatchSuggestionView(View):
    ...
    def update_buttons(self):
        ...
        action_types = (AcceptMatchButton, DeclineMatchButton, CancelMatchButton)
        ...
        # Show Accept and Decline for new matches or recently declined matches
        if not (has_accepted_match or has_pending_request):
            self.add_item(AcceptMatchButton())
            self.add_item(DeclineMatchButton())
        # Show Cancel for accepted or pending
        if has_accepted_match or has_pending_request:
            self.add_item(CancelMatchButton())
...
class PreviousButton(Button):
    ...
    async def callback(self, interaction: Interaction):
        ...
        if view.current_index > 0:
            ...
            if suggestion:
                embed = create_suggestion_embed(suggestion, view.current_index + 1, len(view.suggestions))
...
class DeclineMatchButton(Button):
    """Button to decline a match suggestion."""
    ...
    async def callback(self, interaction: Interaction):
        ...
        # Remove the declined suggestion
        ...
        if not view.suggestions:
            ...
            embed = Embed(
                ...
                description="You've declined all available matches. Try finding matches again later.",
                color=Color.orange()
            )
...
class CancelMatchButton(Button):
    ...
    async def callback(self, interaction: Interaction):
        ...
        try:
            ...
            # Find the most recent match with the appropriate status
            if has_accepted_match:
                target_matches = [m for m in matches if m.status == "scheduled"]
                action_type = "cancelled"
                success_message = "Match has been cancelled and all players have been notified."
            else:  # has_pending_request
                target_matches = [m for m in matches if m.status == "pending_confirmation"]
                action_type = "cancelled"
                success_message = "Match request has been cancelled and all players have been notified."
            
            if not target_matches:
                await interaction.response.send_message(f"No {action_type} match found to cancel.", ephemeral=True)
                return
            
            match = max(target_matches, key=lambda m: m.created_at)
            ...
            # Update the view to reflect the cancellation
            view.update_buttons()
...
def create_suggestion_embed(suggestion: MatchSuggestion, current: int, total: int) -> Embed:
...
class MatchConfirmationView(View):
        ...
        try:
            ...
            embed.add_field(
                ...
                value=(
                    ...
                    "• View match details with `/matches view {match.match_id}`\n"
                    "• Update results after playing with `/matches complete`"
                ),
                inline=False
            )
    ...
    @nextcord.ui.button(label="❌ Decline Match", style=ButtonStyle.danger, custom_id="decline_match")
    async def decline_match(self, button: Button, interaction: Interaction):
        ...
        try:
            ...
            updated_match = self.match_dao.update_match(
                ...
                match.match_id,
                status="cancelled",
                cancelled_reason=f"Declined by {interaction.user.display_name or interaction.user.name}"
            )
            ...
            embed = Embed(
                ...
                description="You have declined this match request.",
                color=Color.red()
            )
